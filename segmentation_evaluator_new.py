...
class NoduleSegmentEvaluator:
    ...
    def evaluate(
        ...
        # slice_identification_r, nodule_identification_r, nodule_dices_r, matched_volumes_r = (
        #     self.get_nodule_wise_metrics(
        #         instance_segmentation, predicted_instance_segmentation, pixel_spacing, slice_thickness,
        #     )
        # )

        # matched_volumes = self.compute_matched_volumes(
        #     predicted_instance_segmentation, instance_segmentation
        # )
        ...
        metrics = {
            ...
            # "slice_identification_r": slice_identification_r,
            # "nodule_identification_r": nodule_identification_r,
            # "nodule_dices_r": nodule_dices_r,
            # "num_pred_volumes": len(nodule_dices_r),
        }
    ...
    def get_nodule_wise_metrics(
        ...
        tuple
            ...
            nodule_identification: whether the nodule is identified at some level of overlap
            nodule_dices: dice coefficient for each nodule, excluding other predicted nodules
        """
        nodule_ids = np.arange(
            1, np.max(true_segmentation) + 1
        )  # np.unique(true_segmentation)
        ...
        for nodule_id in nodule_ids:
            ...
            # does prediction highlight the nodule slice
            is_frames_overlap = self.get_slice_identification(
                predicted_segmentation, nodule_mask
            )
            slice_identification.append(is_frames_overlap)

            predicted_nodule_mask = predicted_segmentation * nodule_mask
            overlapping_predicted_nodule_id = np.bincount(
                predicted_nodule_mask.flatten()
            )

            if len(overlapping_predicted_nodule_id) == 1: # no match
                nodule_dices.append(0)
                nodule_identification.append(False)
                predicted_nodule_id=None

            else:
                predicted_nodule_id = (np.argmax(overlapping_predicted_nodule_id[1:]) + 1).item()
                predicted_nodule_mask = predicted_segmentation == predicted_nodule_id

                # does prediction identify the nodule
                percent_overlap = (predicted_nodule_mask.sum() / nodule_mask.sum()).item()
                nodule_identification.append(
                    percent_overlap > self.nodule_identification_threshold
                )

                # calculate dice for that nodule: TODO decide on voxel size
                nodule_dice = self.get_scan_wise_dice(
                    predicted_nodule_mask, nodule_mask
                )
                nodule_dices.append(nodule_dice)

            vols = self.compute_matched_volume(predicted_nodule_mask, nodule_mask, overlapping_predicted_nodule_id, pixel_spacing, slice_thickness)
            vols["nodule_id"] = int(nodule_id)
            vols["predicted_nodule_id"] = predicted_nodule_id
            matched_volumes.append(vols)
    ...
    def get_slice_identification(
        ...
        # nodule_frames = np.where(true_segmentation > 0)
        # nodule_frames = np.unique(nodule_frames[1])
        # predicted_frames = np.where(predicted_segmentation > 0)
        # predicted_frames = np.unique(predicted_frames[1])
        # is_frames_overlap = np.intersect1d(nodule_frames, predicted_frames).shape[0] > 0
        # return is_frames_overlap

    def compute_matched_volume(
        self, predicted_segmentation: np.ndarray, true_segmentation: np.ndarray, overlapping_predicted_nodule_id:bool, pixel_spacing: float, slice_thickness: float,
    ) -> List[dict]:
        """
        Compute volumes of matched nodules.
        ...
        if len(overlapping_predicted_nodule_id) == 1: # no match
            true_volume = compute_volume_voxel_count(
                true_segmentation, pixel_spacing, slice_thickness,
            )
            predicted_volume = 0
            matched_volumes = {
                    "true_volume": true_volume,
                    "predicted_volume": predicted_volume,
                    "true_slice_range": get_nonzero_slices(true_segmentation),
                    "predicted_slice_range": [],
                }
            ...
            predicted_volume = compute_volume_voxel_count(
                predicted_segmentation, pixel_spacing, slice_thickness,
            )
            ...
            matched_volumes = {
                    ...
                    "predicted_volume": predicted_volume,
                    "true_slice_range": get_nonzero_slices(true_segmentation),
                    "predicted_slice_range": get_nonzero_slices(predicted_segmentation),
                }
            
        return matched_volumes
        
    # def compute_matched_volumes(
    #     self, predicted_segmentation: np.ndarray, true_segmentation: np.ndarray
    # ) -> List[dict]:
    #     """
    #     Compute volumes of matched nodules.
    ...
    #         matched volumes
    #     """
    #     nodule_ids = np.arange(1, np.max(true_segmentation) + 1)
    #     nodule_ids = nodule_ids[nodule_ids != 0]
    #     matched_volumes = []
    #     for nodule_id in nodule_ids:
    #         nodule_mask = true_segmentation == nodule_id
    #         predicted_nodule_mask = predicted_segmentation * nodule_mask
    #         # label of the predicted nodule
    #         overlapping_predicted_nodule_id = np.bincount(
    #             predicted_nodule_mask.flatten()
    #         )
    #         if len(overlapping_predicted_nodule_id) == 1:
    #             true_volume = compute_volume_voxel_count(
    #                 nodule_mask, self.pixel_spacing, self.slice_thickness
    #             )
    #             predicted_volume = 0
    ...
    #             )
    #         else:
    #             predicted_nodule_id = np.argmax(overlapping_predicted_nodule_id[1:]) + 1
    #             predicted_nodule_mask = predicted_segmentation == predicted_nodule_id
    ...
    #             )
    #             predicted_volume = compute_volume_voxel_count(
    #                 predicted_nodule_mask, self.pixel_spacing, self.slice_thickness
    #             )
    ...
    #                     "predicted_volume": predicted_volume,
    #                     "nodule_id": nodule_id,
    #                     "predicted_nodule_id": predicted_nodule_id.item(),
    #                 }
    #             )
    #     return matched_volumes
