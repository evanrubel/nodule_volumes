...
import sys
import copy
from segmentation_evaluator_new import NoduleSegmentEvaluator, compute_volume_voxel_count
...
def deregister_moving_mask(
    ...
    inverse_warp_path = os.path.join(transforms_dir, f"{target_pid}_T{moving_timepoint}_registered_to_T{fixed_timepoint}1InverseWarp.nii.gz")
...
#     mask: np.ndarray, pixel_spacing: tuple[float], slice_thickness: float,
# ) -> float:
#     """
...
def main():
    ...
    # group_num = 1 # done
    # group_num = 2 # done (+ 29)
    # group_num = 3 # (4 left) + 11
    # group_num = 4 # done
    # group_num = 5 # done
    # group_num = 6 # (9 left) + 41
    # group_num = 7 # done
    group_num = 8 # [28 left] (RBG 5 now, RBG 2 third, RBGQ 1 second) running still (+ 26)
    # group_num = 9 # (2 left) + 24
    # group_num = 10 # done
    # group_num = 11 # done
    # group_num = 12 # done (+ 42)
    # group_num = 13 # done
    # group_num = 14 # (4 left) running still
    # group_num = 15 # done (+ 42)
    ...
    for pid in tqdm(sorted(list(pid_to_exam_by_timepoint.keys()))[start_ix+26:stop_ix]):
        if pid in output:
